...
python main_ddp.py --arch $arch --master-ip $master_ip --rank $rank --reducer $reducer --batch-size $bsize --dataset-location $dataset_location --device $device --log-file $log_file --reducer-param $reducer_param --num-workers $num_workers --s3-prefix $s3_prefix
